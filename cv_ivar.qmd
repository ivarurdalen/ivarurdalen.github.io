...
col_br_key <- function(col){
    dplyr::if_else(
      !is.na(col) & col != "N/A|NA", 
      paste0("<br> **Key achievements:** ", col, ""),
      ""
    )
}
    ...
    dplyr::mutate(
      main_text =
        glue::glue(
          ...
          "**{title}** {col_br(institution)} {col_br(description_1)} {col_br_loc(loc)}"),
      .after = date
    ) |>
...
Resume made with 💜 and [Quarto](https://quarto.org/). Last updated on `r format.Date(Sys.Date(), "%b %d, %Y")`. <br> Code available on <i class="fa-brands fa-github"></i> [GitHub](https://github.com/ivarurdalen/). <br>License: [CC BY-SA 2.0](https://creativecommons.org/licenses/by-sa/2.0/).
